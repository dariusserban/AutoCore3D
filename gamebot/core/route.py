...
# Tipurile de reper. Botul se comporta diferit in fiecare zona: la 'combat'
# cauta mob-uri, la 'gather' cauta noduri, la 'vendor' deschide meniuri.
WAYPOINT_KINDS = ("travel", "combat", "gather", "vendor", "repair", "idle", "safe")
...
class Route:
    ...
    def anchor_path(self, waypoint: Waypoint) -> Optional[Path]:
        if not waypoint.anchor or self.directory is None:
            return None
        return self.directory / waypoint.anchor
